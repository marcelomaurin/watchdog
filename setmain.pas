...
begin
    ...
    FHeight :=400;
    FWidth :=400;
    FURLServidor := '';   //Servidor de Banco de dados
    ...
    FProcessName := '';
    FProcessPath := '';
    FTime := '00:05'; //Valor padrao de 5 min
    FWatchdog := true;
...
begin
    ...
    if  BuscaChave(arquivo,'PROCESSNAME:',posicao) then
    begin
          FProcessName := RetiraInfo(arquivo.Strings[posicao]);
    end;
    if  BuscaChave(arquivo,'PROCESSPATH:',posicao) then
    begin
          FProcessPath := RetiraInfo(arquivo.Strings[posicao]);
    end;
    if  BuscaChave(arquivo,'TIME:',posicao) then
    begin
          FTIME := RetiraInfo(arquivo.Strings[posicao]);
    end;
    if  BuscaChave(arquivo,'WATCHDOG:',posicao) then
    begin
      FWatchdog := strtoBool(RetiraInfo(arquivo.Strings[posicao]));
    end;
...
begin
  ...
  arquivo.Append('PROCESSNAME:'+FProcessName);
  arquivo.Append('PROCESSPATH:'+FProcessPath);
  arquivo.Append('TIME:'+FTIME);
  arquivo.Append('WATCHDOG:'+booltostr(FWatchdog));
